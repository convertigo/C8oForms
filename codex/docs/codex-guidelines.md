...
- Scope all structural changes to `_c8oProject/…` (sequences, mobile pages, shared components, etc.) unless explicitly requested **and** apply these changes exclusively through the Convertigo MCP (e.g. `databaseobject-create`, `databaseobject-properties-set`, `project-save`). Do not touch the YAML files with `apply_patch`, local editors, or ad‑hoc scripts.
...
- MCP saves (`autoSave:true` or explicit `project-save`) update the YAML files for us, so do **not** run the legacy CLI reload/compile chain anymore. Focus on validating the MCP response (no errors, expected summary) and continue with the task; Convertigo Studio will pick up the persisted YAML automatically.
...
- **Memory workflow**: avant chaque tâche Convertigo, interroge la base vectorielle (MCP `memory.search`) en filtrant sur les métadonnées pertinentes (`topic: "convertigo"`, `source`, etc.). Toute nouvelle compréhension (procédure MCP, correctif spécifique, subtilité d’API) doit être consignée dans un fichier Markdown du dépôt (`codex/docs` ou `ChromaDB/data`) puis ingérée via `npm run ingest -- --file=CHEMIN --collection=memory` afin que les futurs contextes bénéficient de la mise à jour.
- **Documentation exhaustive**: tout élément nécessaire pour installer, comprendre ou étendre un projet Convertigo (structure des sequences, conventions d’actions, scripts MCP, pièges connus) doit être écrit dans ces documents puis synchronisé dans la base vectorielle. L’objectif est qu’un agent démarre sur un environnement “vierge” (aucun historique de chat ni exemples locaux) et puisse malgré tout reconstituer l’application uniquement grâce aux guides et aux recherches `memory.search`.
